...
function DestinationDetailsComponent({ destinations }) {
  ...
  const { name, description, location, more, best_time_to_visit, traveling_tips, attractions } = destination;

  return (
    <div className="container mt-4" style={{ backgroundColor: "#f0f0f0", padding: "20px" }}>
      <h2 className="display-4 mb-4">{name}</h2>
      <h4 className="mb-3" style={{ color: "brown" }}>Location: {location}</h4>
